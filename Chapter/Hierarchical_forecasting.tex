...
Accurate forecasting of key macroeconomic variables such as GDP, inflation, industrial production, has been in the forefront of economic research over many decades. Early approaches involved univariate models or at best low dimensional multivariate systems. The era of big data has led to now using regularization and shrinkage methods such as dynamic factor models, Lasso, LARS, Bayesian VARs, in an effort to exploit the plethora of potentially useful predictors now available. These predictors commonly also include the components of the variables of interest something ignored by the aforementioned approaches. For example, GDP is the aggregate of consumption, government expenditure, investment and net exports. Each of these components are aggregates of more sub-components with all these all being included in the set of predictors for GDP. 

In this chapter we take a different approach. We directly account for take advantage of the aggregation structure macroeconomic variables of interest and there components form. Such aggregation structures for time are labelled as hierarchical times series.
...
Figures \ref{fig:TS-Inc} and \ref{fig:TS-Exp} display selected time series from the income and  expenditure approaches respectively. The first panel of each figure shows the most aggregate GDP series \textcolor{red}{(I guess this is the same for each figure? Do we need them both? Can we do something else here. Maybe have GPD at the top and then on the left side/panels the income series and on the right side the expenditure series all on the same graph)}. Panel closer to the bottom of these figures show series further down in the hierarchy. The plots show the diverse features of the time series with some displaying positive and others negative trending behaviour, some showing no trends but possibly a cycle, and some having a strong seasonal component. These highlight the need to use all information accounting for diverse signals in each series which can be achieved through a forecast reconciliation approach.
...
We now demonstrate the potential for reconciliation methods to improve forecast accuracy for Australian GDP data.  We consider forecasts from $h=1$ quarter ahead forecasts up to $h=4$ quarter ahead and use an {\em expanding} window.  First, the training sample is set to 1984:Q4 to XXXX:QX and forecasts are produced for XXXX:QX to XXXX:QX. Then the training window is expanded one period ahead keeping the same origin, i.e. from 1984:Q4 to XXXX:QX with forecasts produced for QX of XXXX to QX of XXXX.  All up this leads to XX forecasts.
...
{\color{red} Tas thinks two 4x2 panels would be better}
...
\textcolor{red}{Possible discuss skill scores in more detail especially w.r.t how they are combined}
