...
# Expenditure Approach - Point forecasting
...
SS.GDPE_ETS_MASE <- Skill.Score_GDPE_ETS %>% dplyr::select(`F-method`, `R-method`, `Forecast Horizon`, `SS_MASE` ) %>% 
...
GDPE_PointF <- cbind(SS.GDPE_ETS_MSE, SS.GDPE_ETS_MASE[,-1], SS.GDPE_ARIMA_MSE[,-1], SS.GDPE_ARIMA_MASE[,-1]) %>% 
...
# Expenditure approach - Probabilistic forecasting (Non-parametric Bootstrap approach)

```{r echo=FALSE}

load("Expenditure-approach/ProbForecasting-BootstrapApproach-ExpandingW.RData")
...
Skill.Score_GDPE_ETS %>% dplyr::select(-`MCRPS`, -`Series`) %>% 
...
Skill.Score_GDPE_ARIMA %>% dplyr::select(-`MCRPS`, -`Series`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
...
GDPE_ProbF_NonPara <- cbind(GDPE.ETS_CRPS, GDPE.ARIMA_CRPS[,-1]) %>% 
  kable(format = "latex") %>% kable_styling("striped") %>% 
...
# Expenditure approach - Probabilistic forecasting (Parametric approach assuming Gaussianity)

```{r echo=FALSE}

load("Expenditure-approach/ProbForecasting-GaussianApproach-ExpandingW.RData")
...
DF_MultScore_SS_ETS_LS %>% ungroup() %>% select(-`F-method`) %>% rename("Method" = "R-method") %>%
  mutate(`Method` = replace(`Method`, c(1, 2), c("Bottom-up", "MinT Shrink"))) %>%
  slice(match(Method_Order[3:6], `Method`)) -> ETS_LS

DF_MultScore_SS_ARIMA_LS %>% ungroup() %>% select(-`F-method`) %>% rename("Method" = "R-method") %>%
  mutate(`Method` = replace(`Method`, c(1, 2), c("Bottom-up", "MinT Shrink"))) %>%
  slice(match(Method_Order[3:6], `Method`)) -> ARIMA_LS
...
Skill.Score_GDPE_ETS %>% dplyr::select(-`MCRPS`, -`MLS`, -`SS_CRPS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_LS) %>% ungroup() %>% dplyr::select(-`F-method`, -`Series`) %>%  
  mutate(`R-method` = replace(`R-method`, 1, "Benchmark")) %>% rename("Method" = "R-method") %>%
  slice(match(Method_Order, `Method`)) -> GDPE.ETS_LS
...
Skill.Score_GDPE_ARIMA %>% dplyr::select(-`MCRPS`, -`MLS`, -`SS_CRPS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_LS) %>% ungroup() %>% dplyr::select(-`F-method`, -`Series`) %>%  
  mutate(`R-method` = replace(`R-method`, 6, "Benchmark")) %>% rename("Method" = "R-method") %>%
  slice(match(Method_Order, `Method`)) -> GDPE.ARIMA_LS

GDPE_ProbF_Gaus <- cbind(GDPE.ETS_CRPS, GDPE.ETS_LS[,-1], GDPE.ARIMA_CRPS[,-1], GDPE.ARIMA_LS[,-1]) %>% 
  kable(format = "latex") %>% kable_styling("striped") %>% 
