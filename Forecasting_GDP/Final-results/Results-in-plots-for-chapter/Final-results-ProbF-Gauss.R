...
## Gaussian-Probabilistic-forecasting - Income approach
rm(list = ls())
...
GDPI.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("GDP(I)") -> INC_GDPI_ProbF_Gauss 
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Inc)[1:6])) %>%
  ...
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_INC_aggregates_ARIMA
...
Score_INC_aggregates_ARIMA %>% dplyr::filter(`F-method`=="ARIMA", `R-method`=="Base") %>%
  slice() %>% 
  ungroup() %>%
  dplyr::select(`Avg_LS`) %>% as_vector() -> INC_ARIMA.base_LS

Score_INC_aggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/INC_ARIMA.base_CRPS))*100, digits = 2), 
                                  SS_LS = round((1-(`Avg_LS`/INC_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_INC_aggregates_ARIMA  


Skill.Score_INC_aggregates_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
...
INC_Aggregates.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all aggregate levels") -> INC_Aggregate_ProbF_Gauss 
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Inc)[7:16])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_INC_disaggregates_ARIMA
...
Score_INC_disaggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/INC_ARIMA.base_CRPS))*100, digits = 2), 
                                      SS_LS = round((1-(`Avg_LS`/INC_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_INC_disaggregates_ARIMA  


Skill.Score_INC_disaggregates_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
  mutate(`R-method` = replace(`R-method`, c(7,2), c("Benchmark", "Bottom-up"))) %>% rename("Method" = "R-method") %>%
...
INC_Disaggregates.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all bottom level series") -> INC_Disaggregate_ProbF_Gauss 
...
Score_arima %>% dplyr::select(-`Series`) %>% 
  ...
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_INC_All.series_ARIMA
...
Score_INC_All.series_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/INC_ARIMA.base_CRPS))*100, digits = 2), 
                                         SS_LS = round((1-(`Avg_LS`/INC_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_INC_All.series_ARIMA  


Skill.Score_INC_All.series_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
  mutate(`R-method` = replace(`R-method`, c(7,2), c("Benchmark", "Bottom-up"))) %>% rename("Method" = "R-method") %>%
...
INC_All.series.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all levels") -> INC_All.series_ProbF_Gauss 


ggarrange(INC_GDPI_ProbF_Gauss, INC_Aggregate_ProbF_Gauss, INC_Disaggregate_ProbF_Gauss, INC_All.series_ProbF_Gauss, 
...
## Gaussian-Probabilistic-forecasting - Expenditure approach
rm(list = ls())
...
Score_GDPE_ARIMA %>% dplyr::filter(`F-method`=="ARIMA", `R-method`=="Base") %>%
  slice() %>% 
  ungroup() %>%
  dplyr::select(`MLS`) %>% as_vector() -> GDPE_ARIMA.base_LS

Score_GDPE_ARIMA %>% mutate(SS_CRPS = round((1-(`MCRPS`/GDPE_ARIMA.base_CRPS))*100, digits = 2), 
                            SS_LS = round((1-(`MLS`/GDPE_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_GDPE_ARIMA 


Skill.Score_GDPE_ARIMA %>% dplyr::select(-`MCRPS`, -`MLS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`, -`Series`) %>%  
...
GDPE.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("GDP(E)") -> EXP_GDPE_ProbF_Gauss 
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Exp)[1:27])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_EXP_aggregates_ARIMA
...
Score_EXP_aggregates_ARIMA %>% dplyr::filter(`F-method`=="ARIMA", `R-method`=="Base") %>%
  slice() %>% 
  ungroup() %>%
  dplyr::select(`Avg_LS`) %>% as_vector() -> EXP_ARIMA.base_LS

Score_EXP_aggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/EXP_ARIMA.base_CRPS))*100, digits = 2), 
                                      SS_LS = round((1-(`Avg_LS`/EXP_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_EXP_aggregates_ARIMA  


Skill.Score_EXP_aggregates_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
  mutate(`R-method` = replace(`R-method`, c(6,2), c("Benchmark", "Bottom-up"))) %>% rename("Method" = "R-method") %>%
...
EXP_Aggregates.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all aggregate levels") -> EXP_Aggregate_ProbF_Gauss 
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Exp)[28:80])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_EXP_disaggregates_ARIMA
...
Score_EXP_disaggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/EXP_ARIMA.base_CRPS))*100, digits = 2), 
                                         SS_LS = round((1-(`Avg_LS`/EXP_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_EXP_disaggregates_ARIMA  


Skill.Score_EXP_disaggregates_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
  mutate(`R-method` = replace(`R-method`, c(6,2), c("Benchmark", "Bottom-up"))) %>% rename("Method" = "R-method") %>%
...
EXP_Disaggregates.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all bottom level series") -> EXP_Disaggregate_ProbF_Gauss 
...
Score_arima %>% dplyr::select(-`Series`) %>% 
  group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 2), Avg_LS = round(mean(`MLS`), digits = 2)) -> Score_EXP_All.series_ARIMA
...
Score_EXP_All.series_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/EXP_ARIMA.base_CRPS))*100, digits = 2), 
                                      SS_LS = round((1-(`Avg_LS`/EXP_ARIMA.base_LS))*100, digits = 2)) -> Skill.Score_EXP_All.series_ARIMA  


Skill.Score_EXP_All.series_ARIMA %>% dplyr::select(-`Avg_CRPS`, -`Avg_LS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`) %>%  
  mutate(`R-method` = replace(`R-method`, c(6,2), c("Benchmark", "Bottom-up"))) %>% rename("Method" = "R-method") %>%
...
EXP_All.series.ARIMA_CRPS %>% gather(-Method, key = "h", value = "Score") %>% 
  ...
  scale_color_manual(values = c("green3", "Blue", "Purple", "brown")) +
  scale_shape_manual(values = 0:3) +
  ylab("Skill score %") + ggtitle("Across all levels") -> EXP_All.series_ProbF_Gauss 


ggarrange(EXP_GDPE_ProbF_Gauss, EXP_Aggregate_ProbF_Gauss, EXP_Disaggregate_ProbF_Gauss, EXP_All.series_ProbF_Gauss, 
          ncol=2, nrow=2, common.legend = TRUE, legend="bottom")
