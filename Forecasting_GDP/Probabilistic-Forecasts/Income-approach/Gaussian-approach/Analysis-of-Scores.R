...
Score_ets %>% dplyr::filter(`Series` %in% c(names(Inc)[1:6])) %>%
  ...
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_aggregates_ETS
...
Score_aggregates_ETS %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ETS.base_CRPS))*100, digits = 4), 
                                SS_LS = round((1-(`Avg_LS`/ETS.base_LS))*100, digits = 4)) -> Skill.Score_aggregates_ETS  
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Inc)[1:6])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_aggregates_ARIMA
...
Score_aggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ARIMA.base_CRPS))*100, digits = 4), 
                                  SS_LS = round((1-(`Avg_LS`/ARIMA.base_LS))*100, digits = 4)) -> Skill.Score_aggregates_ARIMA  
...
Score_ets %>% dplyr::filter(`Series` %in% c(names(Inc)[7:16])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_disaggregates_ETS
...
Score_disaggregates_ETS %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ETS.base_CRPS))*100, digits = 4), 
                                SS_LS = round((1-(`Avg_LS`/ETS.base_LS))*100, digits = 4)) -> Skill.Score_disaggregates_ETS  
...
Score_arima %>% dplyr::filter(`Series` %in% c(names(Inc)[7:16])) %>%
  dplyr::select(-`Series`) %>% group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_disaggregates_ARIMA
...
Score_disaggregates_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ARIMA.base_CRPS))*100, digits = 4), 
                                  SS_LS = round((1-(`Avg_LS`/ARIMA.base_LS))*100, digits = 4)) -> Skill.Score_disaggregates_ARIMA  
...
Score_ets %>% dplyr::select(-`Series`) %>% 
  ...
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_all.series_ETS
...
Score_all.series_ETS %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ETS.base_CRPS))*100, digits = 4), 
                                SS_LS = round((1-(`Avg_LS`/ETS.base_LS))*100, digits = 4)) -> Skill.Score_all.series_ETS  
...
Score_arima %>% dplyr::select(-`Series`) %>% 
  group_by(`F-method`, `R-method`, `Forecast Horizon`) %>%
  summarise(Avg_CRPS = round(mean(`MCRPS`), digits = 4), Avg_LS = round(mean(`MLS`), digits = 4)) -> Score_all.series_ARIMA
...
Score_all.series_ARIMA %>% mutate(SS_CRPS = round((1-(`Avg_CRPS`/ARIMA.base_CRPS))*100, digits = 4), 
                                  SS_LS = round((1-(`Avg_LS`/ARIMA.base_LS))*100, digits = 4)) -> Skill.Score_all.series_ARIMA  
