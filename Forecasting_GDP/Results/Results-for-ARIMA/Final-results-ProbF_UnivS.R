#Results summarised for ARIMA based univariate probabilistic forecasts
...
Skill.Score_GDPI_ARIMA %>% dplyr::select(-`MCRPS`, -`MLS`, -`SS_LS`) %>% 
  ...
  mutate(`R-method` = replace(`R-method`, c(7,2,4), c("Benchmark", "Bottom-up", "MinT(Shrink)"))) %>% rename("Method" = "R-method") %>%
...
Skill.Score_GDPE_ARIMA %>% dplyr::select(-`MCRPS`, -`MLS`, -`SS_LS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`, -`Series`) %>%  
  mutate(`R-method` = replace(`R-method`, c(6,2,3), c("Benchmark", "Bottom-up", "MinT(Shrink)"))) %>% rename("Method" = "R-method") %>%
...
Skill.Score_GDPE_ARIMA %>% dplyr::select(-`MCRPS`) %>% 
  spread(key = `Forecast Horizon`, value = SS_CRPS) %>% ungroup() %>% dplyr::select(-`F-method`, -`Series`) %>%  
  mutate(`R-method` = replace(`R-method`, c(6,2,3), c("Benchmark", "Bottom-up", "MinT(Shrink)"))) %>% 
