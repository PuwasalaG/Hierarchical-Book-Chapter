...
\abstract{Accurate forecasts of macroeconomic data help economic agents and policy makers for align decision making. Considering the aggregation structure of these data, we propose to apply the forecast reconciliation methods to get forecasts that are coherent with the inherent aggregation constraints. While applying the state-of-the-art reconciliation methods for point forecasts, a recent extension of these methods for probabilistic forecasts were also introduced in the macroeconomic setting. Using the Australian GDP we have shown that the forecast reconciliation not only gives coherent forecasts but also improves the overall forecast accuracy in both point and probabilistic frameworks. }
...
\end{equation*}
where $\bm{p} = (p_1,\dots,p_m)'$ is an $m$-dimensional vector consisting of a set of proportions that disaggregate the top-level forecast $\hat{y}_{\text{Tot}, T+h|T}$ to forecasts for the bottom-level series; hence $\bm{p}\hat{y}_{\text{Tot}, T+h|T}=\bm{\hat{\bm{b}}}_{T+h|T}$. These are then aggregated by the summing matrix $\bm{S}$.
